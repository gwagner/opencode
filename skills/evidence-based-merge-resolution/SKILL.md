...
6. This conflicted-merge workflow preserves a resolved integration state even when validation still fails or cannot run: commit the mechanically and evidentially resolved merge, mark validation failed or blocked, and report exact commands, paths, and corrective work. Do not overwrite the checkpoint or change unrelated behavior. A validation failure is not proof that another conflicting version is correct. This exception does not apply to clean merges under `git-main-sync`.
